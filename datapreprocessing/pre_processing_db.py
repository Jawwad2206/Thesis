...
def adjust_timestemps(db):

    db["timestamp"] = pd.to_numeric(db["timestamp"], errors='coerce')
...
def remove_not_matching(list_ml_movies):
    ...
    return ml_movies, nf_movies

def intersection_movies(db, ml_movies, nf_movies):
